...
def main():
    ...
    if key1 == key2:
        ...
        print(f"Sending Rx side the encrypted data of Tx:\n{data1}\n") # Payload that will be sent over

        # User 2 - Receives the message from user 1 and decrypts it
        data_in = data1
        p_in = data_in["modulo"]
        r_in = data_in["Tx_publickey"]
        data_rec2 = data_in["data"]
        print(f"Decrypting Tx message:\n{decrypt_string(data_rec2, decode_public_key(r_in, a2, p_in))}\n")
